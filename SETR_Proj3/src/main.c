...
void led_update_task(void) {
    ...
    while (1) {
        ...
        uint64_t atime_ms = k_uptime_get();
        uint32_t atime_s = atime_ms / 1000;
        uint32_t atime_ms_remainder = atime_ms % 1000;
        ...
        atime_ms = k_uptime_get();
        atime_s = atime_ms / 1000;
        uint32_t anew_time_ms_remainder = atime_ms % 1000;
        printk("LED TASK TOOK %u.%llu s\n\r", anew_time_ms_remainder-atime_ms_remainder);
    }
}
...
int read_temperature_task(void) {
    ...
    while (1) {
        ...
        uint64_t btime_ms = k_uptime_get();
        uint32_t btime_s = btime_ms / 1000;
        uint32_t btime_ms_remainder = btime_ms % 1000;
        ...
        if (verboseMode) {
            printk("Read temperature: %d at time %u.%llu s\n\r", temp, btime_s, btime_ms_remainder);
        }

        //  Tell the PID controller to start working with this new value
        k_sem_give(&sensor_to_controller_sem);

         btime_ms = k_uptime_get();
         btime_s = btime_ms / 1000;
        uint32_t bnew_time_ms_remainder = btime_ms % 1000;
        printk("READ TEMP TOOK %u.%llu s\n\r", bnew_time_ms_remainder-btime_ms_remainder);
    }
...
void pid_controller_task(void) {
    ...
    while (1) {
        ...
        uint64_t ctime_ms = k_uptime_get();
        uint32_t ctime_s = ctime_ms / 1000;
        uint32_t ctime_ms_remainder = ctime_ms % 1000;
        ...
        k_sem_give(&controller_to_heater_sem);
         ...
         ctime_ms = k_uptime_get();
         ctime_s = ctime_ms / 1000;
        uint32_t cnew_time_ms_remainder = ctime_ms % 1000;
        printk("PID TASK TOOK %u.%llu s\n\r", cnew_time_ms_remainder-ctime_ms_remainder);
    }
}
...
void heat_control_task(void) {
    ...
    while (1) {
        ...
        uint64_t dtime_ms = k_uptime_get();
        uint32_t dtime_s = dtime_ms / 1000;
        uint32_t dtime_ms_remainder = dtime_ms % 1000;
        bool verboseMode = rtdb_get_verbose();
        ...
        last_heat_state = heater_state;
         ...
         dtime_ms = k_uptime_get();
         dtime_s = dtime_ms / 1000;
        uint32_t dnew_time_ms_remainder = dtime_ms % 1000;
        printk("HEAT TASK TOOK %u.%llu s\n\r", dnew_time_ms_remainder-dtime_ms_remainder);
    }
}
...
static void uart_cb(const struct device *dev, struct uart_event *evt, void *user_data) {
    int err;
        ...
        uint64_t etime_ms = k_uptime_get();
        uint32_t etime_s = etime_ms / 1000;
        uint32_t etime_ms_remainder = etime_ms % 1000;
    switch (evt->type) {
	    ...
	    default:
         ...
         etime_ms = k_uptime_get();
         etime_s = etime_ms / 1000;
        uint32_t enew_time_ms_remainder = etime_ms % 1000;
        printk("UART CB TOOK %u.%llu s\n\r", enew_time_ms_remainder-etime_ms_remainder);
...
int uart_command_task(void) {
    ...
    while (1) {
        ...
        uint64_t ftime_ms = k_uptime_get();
        uint32_t ftime_s = ftime_ms / 1000;
        uint32_t ftime_ms_remainder = ftime_ms % 1000;
        ...
        if (err) {
         ...
         ftime_ms = k_uptime_get();
         ftime_s = ftime_ms / 1000;
        uint32_t fnew_time_ms_remainder = ftime_ms % 1000;
        printk("CMD PROC TOOK %u.%llu s\n\r", fnew_time_ms_remainder-ftime_ms_remainder);
    }
}
